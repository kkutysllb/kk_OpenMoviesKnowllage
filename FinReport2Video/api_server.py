...
提供 Markdown 上传、视频生成任务管理、视频流式播放接口
...
def _parse_progress(line: str) -> Optional[int]:
    """从日志行解析进度（0-100）。适配 Markdown 处理流程。"""
    ...
    if "Step 2/4" in line:
        return 10
    # Step 3/4  并发处理章节
    if "Step 3/4" in line and "并发处理" in line:
        return 12
    # 并发进度："进度: {done}/{total} 章节完成"
    if "进度:" in line and "章节完成" in line:
        try:
            # 格式："  进度: 3/11 章节完成 (45s 已耗)"
            part = line.split("进度:")[1].strip().split("章节完成")[0].strip()
            ...
            pass
    # Step 3/4 或 Step 4/4  合并视频片段
    if "Step 3/4" in line or "Step 4/4" in line:
        return 85
    # 完成
...
async def parse_chapters(
    ...
    """上传 Markdown 并解析章节标题列表（用于前端动画展示）"""
    filename = file.filename.lower()

    if filename.endswith(".md") or filename.endswith(".markdown"):
        return await _parse_markdown_chapters(file)
    else:
        raise HTTPException(400, "只支持 Markdown 文件")
    ...
    if os.path.exists(output_dir):
        for filename in sorted(os.listdir(output_dir), reverse=True):
            ...
            try:
                ...
                name_no_ext = filename[:-4]  # 去掉 .mp4
                # 取文件名作为 source_name（去掉日期前缀）
                if len(name_no_ext) > 9 and name_no_ext[8] == '_':
                    source_name = name_no_ext[9:]  # 20260320_xxx -> xxx
                else:
                    source_name = name_no_ext
                created_ts = stat.st_mtime
                created_at = datetime.fromtimestamp(created_ts).isoformat()
                videos.append({
                    "filename": filename,
                    "pdf_name": source_name + ".md",
...
def cleanup_task(task_id: str):
    """清理任务相关的所有资源：input Markdown 文件 + temp 临时目录 + output 视频"""
    import shutil
    ...
    # 1. 清理 temp/{input_name}/ 目录
    #    input_path 格式： input/{timestamp}_{filename}.md
    #    main.py 会用 os.path.splitext(os.path.basename(input_path))[0] 作为 temp 子目录名
    #    即 temp/{timestamp}_{filename}/，所以这里直接取带时间戳的完整名。
    if task.pdf_path:
        input_name = os.path.splitext(os.path.basename(task.pdf_path))[0]
        temp_dir = os.path.join(TEMP_DIR, input_name)
        if os.path.exists(temp_dir):
            try:
                shutil.rmtree(temp_dir)
                cleaned.append(f"temp/{input_name}/")
                print(f"    [清理] 已删除临时目录: {temp_dir}")
            except Exception as e:
                errors.append(f"temp/{input_name}/: {e}")
                print(f"    [清理] 删除临时目录失败: {e}")

    # 2. 清理 input/ 中的 Markdown 文件
