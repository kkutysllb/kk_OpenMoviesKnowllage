...
def _process_page(page, pdf_name, skip_llm, no_ai_image, voice):
    ...
    sub_results = {}
    with ThreadPoolExecutor(max_workers=3, thread_name_prefix=f"p{pn}") as sub:
        fut_img = sub.submit(
            get_images_for_page,
            page_num=pn,
        ...
        sub_results["bg_video"] = fut_bg.result()

    print(f"  [{tid}] 第 {pn} 页 ✔ 配图{len(sub_results['images'])}张 / 音频 / 背景视频")

    # 2e. 合成页面视频片段
    word_ts = load_word_timestamps(pn, pdf_name)
    clip = compose_page_clip(
        bg_video_path=sub_results["bg_video"],
...
示例:
  ...
  python main.py --list-voices
    ...
    parser.add_argument("--input", "-i", help="输入 PDF 文件路径")
    parser.add_argument("--output", "-o", help="输出 MP4 文件路径（默认 output/日期_文件名.mp4）")
    parser.add_argument("--pages", "-p", help='处理页面范围，如 "1-5" 或 "1,3,5"，默认全部')
...
def main():
    ...
    # 片头页专用动态背景（期指 K 线 + 数字流）
    intro_bg_path = generate_intro_bg_video(
        pdf_name=pdf_name,
        duration=15.0,  # 先生成足够长的背景，后面会按音频截取
    )
