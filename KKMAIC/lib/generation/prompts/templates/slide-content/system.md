...
- Top: ≥ 50
- Bottom: ≤ {{canvas_height}} - 50  ← **HARD LIMIT = {{canvas_height}} - 50 = 512.5px. Anything below this is INVISIBLE.**
- Left: ≥ 50
- Right: ≤ {{canvas_width}} - 50

> ⚠️ **Content Budget Rule**: The usable vertical space is only **462px** (from top=50 to bottom=512.5). A title (~70px) + subtitle (~46px) + separator (~20px) already uses ~136px, leaving **~326px for body content**. If your key points cannot fit in 326px, you MUST **cut bullet points** or **increase font compression** — never exceed the canvas boundary.
...
- **NO OVERLAP RULE (CRITICAL)**: Images MUST NOT overlap any text or shape elements. The image bounding box `[left, top, left+width, top+height]` must NOT intersect with any text element bounding box. Use a strict two-column layout when placing images alongside text:
  - **Text column**: left=60, width ≈ 420 (occupies left half)
  - **Image column**: left ≈ 530, width ≈ 400 (occupies right half)
  - Never place an image starting at `left < 400` unless there is NO text on the slide
  - If image width is large (>400), reduce it to fit within the right column only
...
4. ✓ All elements are within canvas margins (50px from each edge):
   - **CRITICAL MATH CHECK**: For EVERY element, verify `top + height ≤ {{canvas_height}} - 50`. If any element violates this, you MUST reduce font sizes, shrink elements, remove less important elements, or split content — do NOT let any element overflow the bottom boundary.
   - Similarly: `left + width ≤ {{canvas_width}} - 50`
   - If total content cannot fit within the canvas at readable sizes, **remove the least important elements** or merge bullet points to reduce total height.
