...
export function Stage({
  ...
  const startScreenRecording = useCallback(async () => {
    ...
    try {
      ...
      recorder.onstop = () => {
        try {
          if (recordedChunksRef.current.length === 0) {
            toast.error(t('video.recordingFailed'));
            return;
          }
          ...
          a.href = url;
          a.download = `openmaic-classroom-${new Date()
            .toISOString()
            .replace(/[:.]/g, '-')}.webm`;
      ...
      if (
        ...
        // User cancelled or no screen available — silently ignore
        return;
      }
      toast.error(t('video.recordingFailed'));
  ...
  const sceneViewerHeight = (() => {
    ...
    if (isRecordingScreen) {
      // When recording, hide roundtable and use full height below header
  ...
  return (
    <div
      className="stage-root flex-1 flex overflow-hidden bg-gray-50 dark:bg-gray-900"
      data-screen-recording={isRecordingScreen ? 'true' : 'false'}
    >
      {/* Scene Sidebar */}
      <SceneSidebar
        className="stage-sidebar-root"
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0 relative">
        ...
        {/* Roundtable Area */}
        {mode === 'playback' && (
          <div className="stage-roundtable-root">
            <Roundtable
              mode={mode}
